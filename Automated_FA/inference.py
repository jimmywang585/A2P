...
DEEPSEEK_ALIASES = {"deepseek"}
LOCAL_MODEL_ALIASES = LOCAL_LLAMA_ALIASES | LOCAL_QWEN_ALIASES
ALL_MODELS = list(KNOWN_GPT_MODELS | GPT_OSS_MODELS | LOCAL_MODEL_ALIASES | DEEPSEEK_ALIASES)
...
async def async_main(args, client_or_model_obj, model_type, model_family, model_id_or_deployment, output_filepath):
    """Async main function for batch processing"""
    # Create async client based on model type
    if model_type == 'gpt':
        ...
        async_client = create_async_client(
            'gpt-oss',
            api_key=args.gpt_oss_api_key,
            base_url=args.gpt_oss_api_base
        )
    elif model_type == 'deepseek':
        async_client = create_async_client(
            'deepseek',
            api_key=args.deepseek_api_key,
            base_url=args.deepseek_api_base
        )
    else:
        print(f"Async processing not supported for model type: {model_type}")
        return False
    
    try:
        with open(output_filepath, 'w', encoding='utf-8') as output_file, contextlib.redirect_stdout(output_file):
            print(f"--- Starting Async Analysis: {args.method} ---")
            ...
            if args.method == "all_at_once":
                await all_at_once_async(
                    client=async_client,
                    directory_path=args.directory_path,
                    is_handcrafted=is_handcrafted_bool,
                    model=model_id_or_deployment if model_type in ['gpt-oss', 'deepseek'] else args.model,
                    max_tokens=args.max_tokens,
                    batch_size=args.batch_size,
                    a2p=args.a2p
                )
            elif args.method == "step_by_step":
                await step_by_step_async(
                    client=async_client,
                    directory_path=args.directory_path,
                    is_handcrafted=is_handcrafted_bool,
                    model=model_id_or_deployment if model_type in ['gpt-oss', 'deepseek'] else args.model,
            ...
            print("-" * 20)
            print(f"--- Async Analysis Complete ---")
        
        print(f"Analysis finished. Output saved to {output_filepath}")
        return True
        
    except Exception as e:
        print(f"\n!!! An error occurred during async analysis: {e} !!!", file=sys.stderr)
        return False
...
def main():
    ...
    parser.add_argument(
        "--max_tokens", type=int, default=20000,
        help="Maximum number of tokens for GPT API response. Default: 20000."
    )
    ...
    parser.add_argument(
        "--deepseek_api_base", type=str, default="http://127.0.0.1:8080/v1",
        help="DeepSeek API base URL. Default: 'http://127.0.0.1:8080/v1'."
    )
    parser.add_argument(
        "--deepseek_api_key", type=str, default="WESTLAKENLP",
        help="DeepSeek API key. Default: 'WESTLAKENLP'."
    )
    
    # GPT-OSS-specific arguments
    parser.add_argument(
        "--gpt_oss_api_base", type=str, default="http://192.168.181.22:38011/v1",
        help="GPT-OSS API base URL. Default: 'http://192.168.181.22:38011/v1'."
    )
    parser.add_argument(
        "--gpt_oss_api_key", type=str, default="EMPTY",
        help="GPT-OSS API key. Default: 'EMPTY'."
    )
    
    # Async batch processing arguments
    parser.add_argument(
        "--use_async", action="store_true",
        help="Use async batch processing for API calls (recommended for better throughput)."
    )
    parser.add_argument(
        "--batch_size", type=int, default=48,
        help="Number of concurrent API requests to maintain. Default: 48."
    )
    
    # A2P argument
    parser.add_argument(
        "--a2p", action="store_true",
        help="Use A2P (Abduct-Act-Predict) scaffolding framework for enhanced failure attribution."
    )
    ...
    elif args.model in GPT_OSS_MODELS:
        model_type = 'gpt-oss'
        model_family = 'gpt-oss'
        model_id_or_deployment = GPT_OSS_MODEL_MAP[args.model]
        print(f"Selected GPT-OSS model: {args.model} ({model_id_or_deployment})")
        
        try:
            print(f"Initializing GPT-OSS OpenAI client for endpoint: {args.gpt_oss_api_base}")
            client_or_model_obj = OpenAI(
                api_key=args.gpt_oss_api_key,
                base_url=args.gpt_oss_api_base,
            )
            print(f"Successfully initialized GPT-OSS client for endpoint: {args.gpt_oss_api_base}")
        except Exception as e:
            print(f"Error initializing GPT-OSS client: {e}")
            sys.exit(1)
            
    elif args.model in DEEPSEEK_ALIASES:
        model_type = 'deepseek'
        model_family = 'deepseek'
        model_id_or_deployment = DEEPSEEK_MODEL_MAP[args.model]
        print(f"Selected DeepSeek model: {args.model} ({model_id_or_deployment})")
        
        try:
            print(f"Initializing DeepSeek OpenAI client for endpoint: {args.deepseek_api_base}")
            client_or_model_obj = OpenAI(
                api_key=args.deepseek_api_key,
                base_url=args.deepseek_api_base,
            )
            print(f"Successfully initialized DeepSeek client for endpoint: {args.deepseek_api_base}")
        except Exception as e:
    ...
    a2p_suffix = "_a2p" if args.a2p else ""
    output_filename = f"{args.method}_{args.model.replace('/','_')}{a2p_suffix}{handcrafted_suffix}.txt"
    output_filepath = os.path.join(output_dir, output_filename)
    
    # Check if result file already exists
    if os.path.exists(output_filepath):
        print(f"\n⚠️  Result file already exists: {output_filepath}")
        print("Skipping analysis to avoid overwriting existing results.")
        print("To re-run, please delete or rename the existing file.")
        return
    ...
    # Try async processing first if enabled and supported
    if args.use_async and model_type in ['gpt', 'gpt-oss', 'deepseek']:
        print("\nUsing async batch processing...")
        loop = asyncio.get_event_loop()
        success = loop.run_until_complete(
            async_main(args, client_or_model_obj, model_type, model_family, model_id_or_deployment, output_filepath)
        )
        if success:
            return
    ...
    try:
        with open(output_filepath, 'w', encoding='utf-8') as output_file, contextlib.redirect_stdout(output_file):
            ...
            if model_type == 'gpt':
                ...
                # Convert string is_handcrafted to boolean
                is_handcrafted_bool = args.is_handcrafted == "True"
